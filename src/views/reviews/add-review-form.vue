      ...
      <div class="card-body">
        ...
        <template v-else>
          ...
          <div class="row mt-3 mt-sm-0">
            <label class="col-sm-4 col-form-label">Last finalized transaction</label>
            <div class="col-sm-8 d-flex">
              {{ $strings.formatTime(new Decimal(listingSummary.signer.timestamp).mul(1000).toNumber()) }}
            </div>
          </div>
        ...
        <div class="row mt-3 mt-sm-0">
          ...
          <div class="col-sm-8 d-flex">
            <template v-if="amount !== null">
              <amount asset="currency-DOLLAR" :value="amount"/>
            </template>
            <template v-else>
...
export default {
  ...
  components: {AlertBox, LoadingButton, AccountIdenticon, LoadingSpinner, Stars, Amount, InputAmount},

  props:{
    account: {default: ""},
    listing: {default: ""},
    updateType: {default: "purchase"},
    amount: {default: null},
  },
  ...
  methods:{
    ...
    async submitUpdateAccountSummary(){
      ...
      const data = JSONParse(MyTextDecode(await LibertyTown.accountsSummaries.store(MyTextEncode( JSONStringify({
        ...
        salesAmount: this.accountSummary.salesAmount.plus( this.updateType === 'purchase' ? 0 : (this.amount ? this.amount : this.updateAmount.amount) ),
        ...
        purchasesAmount: this.accountSummary.purchasesAmount.plus( this.updateType === 'purchase' ? (this.amount ? this.amount : this.updateAmount.amount) : 0),
    ...
    async submitUpdateListingSummary(){
      ...
      const data = JSONParse(MyTextDecode(await LibertyTown.listingsSummaries.store(MyTextEncode( JSONStringify({
        ...
        amount: this.listingSummary.amount.plus( this.amount ? this.amount : this.updateAmount.amount ),
      })), (data) => this.$store.state.page.validatorModal.showModal(data))) )
    ...
    async submitAddReview(){
      ...
      const data = JSONParse( MyTextDecode( await LibertyTown.reviews.store( JSONStringify({
        ...
        amount: this.amount ? this.amount : this.updateAmount.amount,
      }), (data) => this.$store.state.page.validatorModal.showModal(data) ) ))
