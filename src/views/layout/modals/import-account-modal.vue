<template>
  <modal ref="modal" title="Import wallet from json file">
    ...
    <template v-slot:body>
      ...
      <div>
        <label class="form-label" for="selectFile">Select .liberty file</label>
        <input class="form-control" id="selectFile" type="file" v-on:change="handleFileUpload" size="1" accept=".liberty" ref="file" />
      </div>

    </template>

    <template v-slot:footer>
      <loading-button :disabled="!walletData" class-custom="btn btn-danger" :submit="handleImportWallet" text="Import now" icon="fas fa-save"/>
  ...
  methods: {
    ...
    handleFileUpload(){
      ...
      try{
        ...
        if (extension !== "liberty") throw `File not supported. Maybe wrong file? `
    ...
    async handleImportWallet(){
      ...
      try{
        ...
        const out = await LibertyTown.settings.manager.importJSON( MyTextEncode(this.walletData) );
        if (out)
          this.$store.dispatch('addToast', {
            type: 'success',
            title: `Imported Successfully`,
            text: `The selected file has been was imported successfully`,
          });
